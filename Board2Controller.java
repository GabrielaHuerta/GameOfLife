...
public class Board2Controller 
{
	...
	@FXML
	public ImageView playerOne;  //image of red car

	@FXML 
	public ImageView playerTwo;  //image of blue car



	@FXML
	public ImageView actionCard; //image of action card

	@FXML
	public ImageView houseCard;  //image of house card
	...
	@FXML
	public Label action;  //label for "Spin" button
	...
	private void action(Player play, ArrayList<Coordinates>list, int loc)
	{
		if(list.get(loc).getIdentifaction().equals("y"))  //if the space is yellow, a new action card will be displayed
		{
			...
			//Lables must be refreshed to display new money totals TACOS
		...
		else if(list.get(loc).getIdentifaction().equals("h"))   //if it is a house space, a new house card will be shown & home info. will be changed 
		{
			...
			if(play.getName().equals(TwoPlayerScreenController.getOne().getName())) //based on player, refreshes label 
